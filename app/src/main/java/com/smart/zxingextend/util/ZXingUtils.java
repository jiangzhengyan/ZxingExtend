package com.smart.zxingextend.util;
...
public class ZXingUtils {
    ...
    /**
     * 同步解析bitmap多码二维码。该方法是耗时操作，请在子线程中调用。
     *
     * @param picturePath 要解析的二维码图片本地路径
     * @return 返回二维码图片里的内容Result[]  或 null
     */
    public static Result[] syncDecodeMultiQRCode(String picturePath) {
        return syncDecodeMultiQRCode(getDecodeAbleBitmap(picturePath));
    }
    /**
     * 同步解析bitmap多码二维码。该方法是耗时操作，请在子线程中调用。
     *
     ...
     * @return 返回二维码图片里的内容Result[]  或 null
     */
    public static Result[] syncDecodeMultiQRCode(Bitmap bitmap) {
        try {
            int width = bitmap.getWidth();
            int height = bitmap.getHeight();
            int[] pixels = new int[width * height];
            bitmap.getPixels(pixels, 0, width, 0, 0, width, height);
            com.google.zxing.RGBLuminanceSource source = new com.google.zxing.RGBLuminanceSource(width, height, pixels);
            Result[] result = new QRCodeMultiReader().decodeMultiple(new BinaryBitmap(new HybridBinarizer(source)), HINTS);
